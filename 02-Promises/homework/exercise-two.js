 ...
 */
  // promise version
  // ???
  let sOne = promisifiedReadFile('poem-two/stanza-01.txt')
  let sTwo = promisifiedReadFile('poem-two/stanza-02.txt')
 //Promise.all([promisifiedReadFile('poem-two/stanza-01.txt'), promisifiedReadFile('poem-two/stanza-02.txt')])
    Promise.all([sOne, sTwo])
   .then(stanzas => {
     blue(stanzas[0]);
     blue(stanzas[1]);
     console.log('done');
...
function problemB () {
  ...
  // promise version
  // ???
  let promiseFiles = filenames.map(f => promisifiedReadFile(f).then(stanza => { blue(stanza) }));
 ...
 */
  ...
  filenames.reduce((acc, current) => {
    return acc.then(stanza => {
      if (stanza) blue(stanza);
      return promisifiedReadFile(current)
    })
